...
get_free_block(gc_t gc, struct s_arena *arena)
{
        ...
        if (__predict_true(SLIST_FIRST(&arena->free_blocks))) {
                ...
                if ((arena->block_used >= arena->block_threshold)) {
                        ...
                        if (__predict_false
                            ((unsigned)arena->block_used * 10 >=
                             arena->block_threshold * 9)) {
        ...
        for (; sc; sc = SLIST_NEXT(sc, next)) {
                ...
                while (pg) {
                        ...
                        if (__predict_false(pg->num_free == 0)) {
                                ...
                                arena->block_used--;
                                VALGRIND_MAKE_MEM_NOACCESS((char *)pg +
                                                           sizeof(struct
                                                                  s_block),
                                                           BLOCK_SIZE -
                                                           sizeof(struct
                                                                  s_block));
                                SLIST_INSERT_HEAD(&arena->free_blocks, pg,
                                                  link);
                        } else {
                                if (!best) {
                                        ...
                                        SLIST_INSERT_HEAD(&sc->blocks, pg,
                                                          link);
                                }
                        }
                        if (!npg && fpg) {
...
clear_block_used_bits(unsigned num_entries, struct s_block *pg)
{
        ...
        memset(pg->used, 0,
               BITARRAY_SIZE_IN_BYTES(num_entries) - sizeof(pg->used[0]));
        int excess = num_entries % BITS_PER_UNIT;
        pg->used[BITARRAY_SIZE(num_entries) - 1] = ~((1UL << excess) - 1);
#if JHC_VALGRIND
        unsigned header =
                sizeof(struct s_block) + BITARRAY_SIZE_IN_BYTES(num_entries);
...
s_alloc(gc_t gc, struct s_cache *sc)
{
        ...
        if (__predict_false(!pg)) {
                ...
                unsigned found =
                        bitset_find_free(&next_free, BITARRAY_SIZE(sc->num_entries),
                                         pg->used);
                pg->next_free = next_free;
                void *val =
                        (uintptr_t *) pg + pg->pi.color + found * pg->pi.size;
...
s_set_used_bit(void *val)
{
        ...
        struct s_block *pg = S_BLOCK(val);
        unsigned int offset =
                ((uintptr_t *) val - (uintptr_t *) pg) - pg->pi.color;
...
static struct s_cache *
find_cache(struct s_cache **rsc, struct s_arena *arena, unsigned short size,
