...
deriveIx :: Derive -> Module -> Data -> Q HsDecl
deriveIx der mod d@D { .. }
    ...
    err s = warn sloc InvalidDecl s >> mkInst der mod d class_Ix []
...
--        (euwn,euw) <- newVar (Just mod)
        ...
        mkInst der mod d class_Ix [range,unsafeIndex,inRange]
    doix ~[b@Body { .. }] = do
        --(pa,[]) <- mkPat mod b
        err "Data.Ix deriving for complex types not yet implemented."
        mkInst der mod d class_Ix []

pair a b = HsTuple [a,b]

deriveEnum :: Derive -> Module -> Data -> Q HsDecl
deriveEnum der mod d@D { .. }
    ...
    where
    sloc = deriveSrcLoc der
    err s = warn sloc InvalidDecl s >> mkInst der mod d class_Enum []
    unit_enum  ~[b@Body { .. }] = do
        (pa,[]) <- mkPat mod b
